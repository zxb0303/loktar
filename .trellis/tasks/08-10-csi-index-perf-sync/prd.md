...
1. 支持多指数配置：指数代码以 `static final` 列表常量定义在任务类顶部，初始包含 `930955`（红利低波100）、`H20955`（红利低波100全收益）。
2. 定时调度：每天 15-23 点，每 10 分钟执行一次（cron：`0 0/10 15-23 * * *`），与现有投资类任务风格一致。
...
- [ ] 新增定时任务类，调度时间 15-23 点每 10 分钟，幂等同步当日行情
